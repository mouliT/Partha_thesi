...
\begin{eqnbox}[Fourier Series Reminder]
Any periodic function \(f(\theta)\) can be expanded as:
\[
...
- \(a_n = 0\) for all \(n\) (cosine terms zero).
- Only sine terms survive: \(f(\theta) = \sum_{n=1,3,\dots} b_n \sin(n\theta)\).

Since \(\sin(n\theta)\) for even \(n\) is even (because \(\sin(-n\theta) = -\sin(n\theta)\) for odd n, wait no:

\(\sin(n\theta)\) for even n: \(\sin(-n\theta) = -\sin(n\theta)\), but since f is odd, the coefficient must make it odd.

Standard result: Odd functions have only odd sine harmonics.
