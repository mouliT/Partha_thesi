...
\textit{This appendix explains why the back-EMF in a PMSM contains only odd-order harmonics (1st, 3rd, 5th, etc.) and no even harmonics (2nd, 4th, etc.). We start from elementary electromagnetism and build to the symmetry argument, referencing standard textbooks.}

\begin{physbox}[Back-EMF Basics: What It Is]
Back-EMF is the voltage induced in the stator windings due to the rotating magnetic field of the rotor. In a PMSM, permanent magnets create a sinusoidal flux density in the air gap. As the rotor spins, this flux links the stator windings, inducing a voltage proportional to the rate of change of flux linkage.

For an ideal sinusoidal machine, the back-EMF is perfectly sinusoidal. But real machines have slight distortions, leading to harmonics. Crucially, even harmonics are absent due to symmetry.
\end{physbox}

\begin{statebox}[Symmetry Argument: The Key Reason]
Consider the back-EMF waveform as a function of rotor position \(\theta\) (electrical angle).

In a balanced three-phase PMSM:
- The stator windings are symmetrically distributed (120° apart).
- The rotor flux is sinusoidal and symmetric.

If we reverse the rotor direction (flip the sign of \(\theta\)), the induced EMF reverses sign: \(e(-\theta) = -e(\theta)\).

This makes the back-EMF an \textbf{odd function} with respect to \(\theta\).

For an odd function, the Fourier series contains \textbf{only odd harmonics}. Even harmonics are zero by symmetry.

Mathematically: \(e(\theta) = \sum_{n=1,3,5,\dots} E_n \sin(n \theta)\).
\end{statebox}

\begin{eqnbox}[Fourier Series Derivation for Odd Functions]
Any periodic function \(f(\theta)\) with period \(2\pi\) can be expanded as:
\[
...
a_n = \frac{1}{\pi} \int_{-\pi}^{\pi} f(\theta) \cos(n\theta) \, d\theta, \quad
b_n = \frac{1}{\pi} \int_{-\pi}^{\pi} f(\theta) \sin(n\theta) \, d\theta.
\]

For an odd function (\(f(-\theta) = -f(\theta)\)):
- The integral for \(a_0\) is \(\frac{1}{2\pi} \int_{-\pi}^{\pi} f(\theta) \, d\theta = 0\), since the integrand is odd.
- For \(a_n\): \(\cos(n\theta)\) is even (\(\cos(-n\theta) = \cos(n\theta)\)), \(f(\theta)\) odd, so odd × even = odd, integral zero.
- For \(b_n\): \(\sin(n\theta)\) is odd (\(\sin(-n\theta) = -\sin(n\theta)\)), \(f(\theta)\) odd, so odd × odd = even, integral nonzero.

Thus, \(f(\theta) = \sum_{n=1}^\infty b_n \sin(n\theta)\).

However, in PMSM back-EMF, further symmetry \(f(\theta + \pi) = -f(\theta)\) ensures only odd \(n\): replace \(\theta \to \theta + \pi\), \(f(\theta + \pi) = \sum b_n \sin(n(\theta + \pi)) = \sum b_n \sin(n\theta + n\pi) = \sum b_n \sin(n\theta) \cos(n\pi) + \cos(n\theta) \sin(n\pi) = \sum b_n (-1)^n \sin(n\theta)\).

Since \(f(\theta + \pi) = -f(\theta) = -\sum b_n \sin(n\theta)\), so \(b_n (-1)^n = -b_n\), thus \(b_n [(-1)^n + 1] = 0\), so \(b_n = 0\) for even \(n\).

Therefore, only odd harmonics: \(f(\theta) = \sum_{n=1,3,5,\dots} b_n \sin(n\theta)\).
\end{eqnbox}

\begin{circuitbox}[Why Even Harmonics Would Require Asymmetry]
Even harmonics (e.g., 2nd) would appear if:
- The rotor had a DC flux bias (uncommon in PMSMs).
- The stator windings were asymmetrically distributed.
- Saturation or other nonlinearities introduced asymmetry.

In the considered machine (balanced PMSM), these are absent, so even harmonics are zero.

This is confirmed in measurements (Fig. 2.2) and standard theory.
\end{circuitbox}

\begin{warningbox}[References]
This argument is standard in electric machine textbooks, e.g.:
- Boldea, I. and Nasar, S.A., 2002. \textit{Electric machines dynamics}. CRC Press, Chapter 3.
- Fitzgerald, A.E., Kingsley Jr, C., and Umans, S.D., 2003. \textit{Electric machinery}. McGraw-Hill, Section 5.5.

For PMSM specifics, see: Bianchi, N., 2006. \textit{Electrical machine analysis using finite elements}. CRC Press.

The Fourier series derivation follows standard signals and systems treatment, e.g.:
- Oppenheim, A.V., Willsky, A.S., and Nawab, S.H., 1997. \textit{Signals and systems}. Prentice Hall, Chapter 3.
\end{warningbox}
...
\textit{This appendix derives the electromagnetic torque equation in PMSMs from elementary principles: force on current-carrying wires in magnetic fields. We build to the standard dq-frame expression, then derive torque ripple from back-EMF harmonics. No prior knowledge of dq transformation assumed — we derive it step by step.}

\begin{physbox}[Elementary Torque: Force on Wires]
Torque in an electric motor comes from magnetic forces on current-carrying conductors.

Imagine a straight wire of length \(l\) carrying current \(i\), in a uniform magnetic field \(\mathbf{B}\). The force is \(\mathbf{F} = i (\mathbf{l} \times \mathbf{B})\).

For a rotating machine, windings are on a cylindrical stator. Torque \(\tau\) is the sum of moments: \(\tau = \sum r \times \mathbf{F}\), where \(r\) is the radial vector.

In PMSMs, the rotor has permanent magnets creating \(\mathbf{B}\), stator windings carry \(i\).
\end{physbox}

\begin{circuitbox}[From Wires to Windings: Flux Linkage]
A stator winding has \(N\) turns, each with flux linkage \(\lambda = \int \mathbf{B} \cdot d\mathbf{A}\).

The induced EMF \(e = -d\lambda/dt\).

Torque from energy: Power input \(P_{elec} = i e\), but for steady state, \(P_{elec} = T \omega +\) losses.

The electromagnetic torque is \(T_{em} = \frac{P_{elec} - P_{loss}}{\omega}\), but approximately \(T_{em} = \frac{i e}{\omega}\) for no losses.

In PMSM, average torque \(T_{avg} = \frac{E I}{\omega}\), where \(E\) is back-EMF RMS, but for exact, we need phasor analysis.
\end{circuitbox}

\begin{statebox}[Phasor Approach: Three-Phase PMSM]
In three-phase PMSM, stator voltages: \(\mathbf{v} = [v_a, v_b, v_c]^T\), currents \(\mathbf{i}\), back-EMFs \(\mathbf{e}\).

The torque \(T = \frac{1}{\omega} (\mathbf{e} \cdot \mathbf{i})\), where \(\omega\) is mechanical speed.

For sinusoidal, \(\mathbf{e} = E \angle \theta\), but with harmonics, expand.

To get the standard expression, transform to dq frame.
\end{statebox}

\begin{eqnbox}[dq Transformation: Why and How]
The dq frame rotates with the rotor, simplifying analysis.

Define dq currents: \(i_d = \frac{2}{3} (i_a \cos\theta + i_b \cos(\theta - 120^\circ) + i_c \cos(\theta - 240^\circ))\), similarly for \(i_q\).

Voltages: \(v_d = \frac{2}{3} (v_a \cos\theta + \dots)\), etc.

For PMSM, the flux linkages: \(\lambda_d = L_d i_d + \psi_m\), \(\lambda_q = L_q i_q\).

EMF: \(e_d = -\omega \lambda_q\), \(e_q = \omega \lambda_d\).

Torque: From power balance, \(T = \frac{3}{2} p (\lambda_d i_q - \lambda_q i_d)\), where p is pole pairs.

For SPM (\(L_d = L_q = L_s\)), \(T = \frac{3}{2} p \psi_m i_q\).
\end{eqnbox}

\begin{circuitbox}[Deriving Torque Ripple]
Torque ripple is the variation around average torque.

If back-EMF has harmonics, \(e_a = \sum_{n=1,3,\dots} E_n \sin(n \theta)\), and currents sinusoidal, torque \(T(\theta) = \frac{1}{\omega} e_a i_a + \dots\)

Each harmonic contributes: \(T_{ripple,n} \propto E_n I \cos(n \delta)\), where \(\delta\) is phase shift.

In multiphase, for ASP vs SSP, the spatial shift \(\delta = 30^\circ\) or \(60^\circ\) causes cancellation for certain \(n\).

For example, 5th harmonic: \(n=5\), \(\delta=30^\circ\), \(5 \times 30 = 150^\circ\), \(\cos(150^\circ) = -\sqrt{3}/2\), but depending on phase, may cancel.

The exact derivation involves expanding in dq frame with harmonic fluxes.

From Lesson 2: \(T_{ripple,n} \propto E_n I_{fundamental} \cos(n \delta)\)
\end{circuitbox}

\begin{statebox}[Key Takeaway]
Electromagnetic torque arises from magnetic forces on stator currents in rotor field. In dq frame, \(T = \frac{3}{2} p (\psi_m i_q + (L_d - L_q) i_d i_q)\).

Torque ripple comes from back-EMF harmonics interacting with currents; multiphase symmetry cancels some harmonics.
\end{statebox}

\begin{statebox}[Checkpoint (Explain in Your Own Words)]
Derive the torque from first principles: how does current in stator wire create torque? Then, explain why harmonics in back-EMF cause torque ripple.
\end{statebox}

% Note: If figures are needed, e.g., a diagram of force on wire or phasor for torque, please indicate.
